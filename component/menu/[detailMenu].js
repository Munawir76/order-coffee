...
export default function DetailMenu() {
    ...
    async function getDataDetailProduct() {
        try {
            const tokenDetailProduct = await localStorage.getItem('tokenCustomer')
    ...
    const dataSelected = dataDetailProduct.find((data) => data.name == detailMenu);
            ...
            {dataSelected.map((menu) => {
                return (
                    <div className='h-screen ml-40 mt-24' style={{ position: "relative" }}>
                        <Row justify='start' >
                            <Col span='8'>
                                <Image src={`https://ordercoffee-app.herokuapp.com/menu/image/${menu.photo}`}
                                    unoptimized={true}
                                    width={350}
                                    height={350}
                            ...
                            <Col style={{ textAlign: 'start', marginLeft: 20 }} span='8'>
                                <h2 className="font-bold text-2xl text-[#805336]">{menu.name}</h2>
                                <h2 className="font-semibold text-xl mt-5 text-gray-500">Rp. {menu.price}</h2>
                        ...
                        <Row justify='start'>
                            <Col span='8'>
                                ...
                                <p className="font-normal text-base mt-5">{menu.description}</p>
